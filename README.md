...
本项目进一步将企业知识库按业务场景拆分为人事制度知识库、财务报销知识库、行政流程知识库和销售话术知识库，并通过问题分类器将用户问题路由到对应知识库。

在财务报销分支中，加入参数提取器与 Code 规则判断节点，从用户问题中提取报销类型和金额，并对打车金额是否超过 200 元进行结构化判断。最终由 LLM 综合知识库检索结果与规则判断结果生成回复，提升金额规则和审批条件类问题的回答稳定性。